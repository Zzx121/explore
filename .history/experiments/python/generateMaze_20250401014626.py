...
for y in range(1, maze_height + 1):
    for x in range(1, maze_width + 1):
        if (x == stat_x and y == start_y):
            maze_list.append({(x, y): start})
        elif (x == end_axis[0] and y == end_axis[1]):
            maze_list.append({(x, y): end})
            ## pathes on x
        elif ((y == start_y and x > min(stat_x, end_axis[0]) and x <= max(stat_x, end_axis[0])) or
              ## pathes on y
              (x == end_axis[0] and y > min(start_y, end_axis[1]) and y < max(start_axis[1], end_axis[1]))):
